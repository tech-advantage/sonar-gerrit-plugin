...
public class GerritPostJob implements PostJob {
	...
	public void execute(PostJobContext postJobContext) {
		...
		try {
			...
			reviewInput.setMessage(ReviewUtils.substituteProperties(
					gerritConfiguration.getMessage(), settings));

			if (LOG.isDebugEnabled()) {
				LOG.debug("[GERRIT PLUGIN] Define message : {}",
						reviewInput.getMessage());
				LOG.debug("[GERRIT PLUGIN] Number of comments : {}",
						reviewInput.size());
			}
			...
			if (LOG.isDebugEnabled()) {
				LOG.debug(
						"[GERRIT PLUGIN] Configured threshold {}, max review level {}",
						gerritConfiguration.getThreshold(),
						ReviewUtils.valueToThreshold(maxLevel));
			}

			if (ReviewUtils.isEmpty(reviewInput)) {
				if (LOG.isDebugEnabled()) {
					LOG.debug(
							"[GERRIT PLUGIN] No issues ! Vote {} for the label : {}",
							gerritConfiguration.getVoteNoIssue(),
							gerritConfiguration.getLabel());
				}
				reviewInput.setValueAndLabel(
						gerritConfiguration.getVoteNoIssue(),
						gerritConfiguration.getLabel());
			} else if (maxLevel < ReviewUtils
					.thresholdToValue(gerritConfiguration.getThreshold())) {
				if (LOG.isDebugEnabled()) {
					LOG.debug(
							"[GERRIT PLUGIN] Issues below threshold. Vote {} for the label : {}",
							gerritConfiguration.getVoteBelowThreshold(),
							gerritConfiguration.getLabel());
				}
				reviewInput.setValueAndLabel(
						gerritConfiguration.getVoteBelowThreshold(),
				...
				if (LOG.isDebugEnabled()) {
					LOG.debug(
							"[GERRIT PLUGIN] Issues above threshold. Vote {} for the label : {}",
							gerritConfiguration.getVoteAboveThreshold(),
							gerritConfiguration.getLabel());
				}
				reviewInput.setValueAndLabel(
						gerritConfiguration.getVoteAboveThreshold(),
			...
			if (LOG.isDebugEnabled()) {
				LOG.debug(
						"[GERRIT PLUGIN] Send review for ChangeId={}, RevisionId={}",
						gerritConfiguration.getChangeId(),
	...
	public void decorate(InputPath resource, PostJobContext context,
		...
		if (!resource.file().isFile()) {
			if (LOG.isDebugEnabled()) {
				LOG.debug("[GERRIT PLUGIN] {} is not a file",
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug(
					"[GERRIT PLUGIN] Look for in Gerrit if the file was under review, name={}",
					resource.relativePath());
		}

		if (gerritModifiedFiles.contains(resource.relativePath())) {
			LOG.info(
					"[GERRIT PLUGIN] Found a match between Sonar and Gerrit for {}",
					resource.relativePath());
			processFileResource(resource.relativePath(), issues);
		} else if (gerritModifiedFiles.contains(gerritFacade
				.parseFileName(resource.relativePath()))) {
			LOG.info(
					"[GERRIT PLUGIN] Found a match between Sonar and Gerrit for {}",
					gerritFacade.parseFileName(resource.relativePath()));
			processFileResource(
					gerritFacade.parseFileName(resource.relativePath()), issues);
		} else {
			if (LOG.isDebugEnabled()) {
				LOG.debug("[GERRIT PLUGIN] File is not under review ({} / {})",
						resource.relativePath(),
						gerritFacade.parseFileName(resource.relativePath()));
			}
		}
	...
	protected void assertOrFetchGerritModifiedFiles()
			throws GerritPluginException {
		if (gerritModifiedFiles != null) {
			return;
		}
		gerritModifiedFiles = gerritFacade.listFiles();
		if (LOG.isDebugEnabled()) {
			LOG.debug("[GERRIT PLUGIN] Modified files in gerrit : {}",
					gerritModifiedFiles);
		}
	}
	...
	protected ReviewLineComment issueToComment(PostJobIssue issue) {
		...
		result.setMessage(ReviewUtils.issueMessage(
				gerritConfiguration.getIssueComment(), settings, issue));
		if (LOG.isDebugEnabled()) {
	...
	protected void processFileResource(@NotNull String file,
			@NotNull Collection<PostJobIssue> issuable) {
	...
	private void commentIssues(Collection<PostJobIssue> issues,
			List<ReviewFileComment> comments) {
		LOG.info("[GERRIT PLUGIN] Found {} issues", issues.size());

		for (PostJobIssue issue : issues) {
			if (gerritConfiguration.shouldCommentNewIssuesOnly()
					&& !issue.isNew()) {
