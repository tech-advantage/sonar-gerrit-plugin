package pl.touk.sonar;

public final class PropertyKey {
    public static final String GERRIT_HOST = "gerrit.host";
    public static final String GERRIT_PORT = "gerrit.port";
    public static final String GERRIT_USERNAME = "gerrit.username";
    public static final String GERRIT_PROJECT = "gerrit.project";
    public static final String GERRIT_CHANGE_ID = "gerrit.changeId";
    public static final String GERRIT_REVISION_ID =" gerrit.revisionId";
}
